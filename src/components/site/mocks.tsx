...
export function Pill({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-2 py-0.5 font-mono text-[10px] tracking-wider uppercase",
        map[tone],
      )}
    >
      <Dot tone={tone} />
...
function SideNav({ items, active, brand }: { items: string[]; active: string; brand: string }) {
  return (
      ...
      {items.map((it) => (
        <span
          ...
          className={cn(
            ...
            it === active ? "bg-ink-fg/8 text-ink-fg" : "text-ink-muted hover:text-ink-fg",
          )}
        >
          {it}
        </span>
      ...
      <div className="min-w-0 flex-1 p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="font-mono text-[10px] text-ink-muted">LAST 30 DAYS · ALL CHANNELS</p>
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="rounded-lg border border-ink-line bg-ink-2/60 p-3">
              <p className="font-mono text-[10px] tracking-wider text-ink-muted uppercase">
                {label}
              </p>
              <p className="mt-1 text-base font-medium tracking-tight sm:text-lg">{value}</p>
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-[1.4fr_1fr]">
          <div className="rounded-lg border border-ink-line bg-ink-2/60 p-3">
            ...
            <Spark className="mt-3" values={[18, 26, 21, 34, 29, 41, 38, 52, 47, 61, 55, 68]} />
        ...
        <div className="mt-3 overflow-hidden rounded-lg border border-ink-line">
          ...
          {heroActivity.map(([title, who, meta, tone], i) => (
            ...
            >
              ...
              <span className="shrink-0 font-mono text-[10px] text-ink-muted">{meta}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-4 p-5 lg:grid-cols-[1.25fr_1fr]">
        <div>
          ...
          <p className="mt-2 max-w-sm text-[13px] text-muted-foreground">
            Personalised physiotherapy programmes, same-week appointments and recovery plans you can
            actually follow.
          </p>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            {["Sports injury", "Post-surgery", "Back & neck"].map((s) => (
              <div key={s} className="rounded-lg border border-border bg-card p-2.5">
                ...
                <p className="font-mono text-[9px] text-muted-foreground">45 MIN · ₹900</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-px bg-ink-line">
          {[
            ...
            <div key={l} className="bg-ink px-3 py-2.5">
              <p className="font-mono text-[9px] tracking-wider text-ink-muted uppercase">{l}</p>
          ...
          <div className="bg-ink">
            ...
            {rows.map(([name, src, status, tone, value]) => (
              ...
              >
                ...
                <span className="text-right font-mono text-[10px] text-ink-muted">{value}</span>
              </div>
            ))}
          </div>
...
export function DiscoverMock() {
  ...
  return (
    <div className="p-5 text-ink-fg">
      ...
      <div className="mt-5 flex items-center gap-2 overflow-x-auto max-[480px]:no-scrollbar">
        {nodes.map(([n, tone], i) => (
          <div
            key={n}
            className="flex min-w-0 flex-1 items-center gap-2 max-[480px]:min-w-[60px] max-[480px]:flex-none"
          >
            <div className="min-w-0 flex-1 rounded-lg border border-ink-line bg-ink-2/60 px-2 py-3 text-center max-[480px]:min-w-[60px]">
              <p className="truncate text-[11px]">{n}</p>
              <span className="mt-1 inline-block">
                <Dot tone={tone as "live"} />
              </span>
            </div>
            {i < nodes.length - 1 && <span className="h-px w-4 shrink-0 bg-ink-line" />}
          </div>
        ))}
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        active ? "border-brand bg-brand/6" : "border-ink-line bg-ink-2/50",
      )}
    >
      <div className="flex items-center justify-between border-b border-ink-line/70 px-2 py-1">
        <span className="truncate text-[7.5px] font-medium text-ink-fg/80">{label}</span>
        <span className="flex gap-0.5">
          <span className="size-[3px] rounded-full bg-ink-fg/20" />
          <span className="size-[3px] rounded-full bg-ink-fg/20" />
        </span>
      </div>
...
export function DesignMock() {
  return (
    <div className="p-5 text-ink-fg">
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        <DesignScreenFrame label="Home" active>
          <p className="text-[8px] leading-tight font-medium text-ink-fg">
            Care built around your recovery
          </p>
          <span className="mt-2 inline-block rounded bg-brand px-1.5 py-0.5 font-mono text-[6.5px] text-primary-foreground">
            Book a visit
          </span>
        </DesignScreenFrame>
        <DesignScreenFrame label="Services">
          <div className="space-y-1">
            {["Back pain", "Sports injury", "Post-surgery"].map((s) => (
              <div key={s} className="flex items-center gap-1 rounded bg-ink-fg/6 px-1.5 py-1">
                <span className="size-1 shrink-0 rounded-full bg-brand" />
                <span className="truncate text-[6.5px] text-ink-muted">{s}</span>
              </div>
            ))}
          </div>
        </DesignScreenFrame>
        <DesignScreenFrame label="Booking">
          <div className="space-y-1">
            <div className="flex h-3.5 items-center rounded border border-ink-line bg-ink-2/40 px-1.5 text-[6px] text-ink-muted">
              Full name
            </div>
            <div className="flex h-3.5 items-center rounded border border-ink-line bg-ink-2/40 px-1.5 text-[6px] text-ink-muted">
              Preferred date
          ...
          <span className="mt-1.5 inline-block rounded bg-brand px-1.5 py-0.5 font-mono text-[6.5px] text-primary-foreground">
            Confirm
          </span>
        </DesignScreenFrame>
      </div>
  ...
  return (
    <div className="grid gap-px bg-ink-line lg:grid-cols-2">
      ...
      <div className="space-y-2 bg-ink p-4">
        <p className="font-mono text-[10px] tracking-wider text-ink-muted uppercase">Milestone 3</p>
        {[
          ["Booking engine", 100],
          ["Payments", 78],
          ["Admin console", 46],
  ...
  return (
    <div className="h-full bg-ivory-2 text-ink">
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        ...
        <span className="rounded bg-ink px-2 py-0.5 font-mono text-[9px] text-ink-fg">Book</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-px border-t border-border bg-border">
        {["Treatments", "Team", "Contact"].map((s) => (
          <div
            key={s}
            className="bg-ivory-2 px-2 py-1.5 text-center font-mono text-[9px] text-muted-foreground"
          >
            {s}
          </div>
        ))}
